...
<script>
	export default {
		...
		computed: {
			...
			rows: function () {
				if (this.current.grouping.length) {
					return this.groupRows(this.data);
				}
				return this.data;
			}
		...
		methods: {
			groupRows: function (rows) {
				let grouping = this.current.grouping;
				let result =  this.groupBy(this.data, grouping[0]);
				for (let i = 1; i < grouping.length; i++) {
					for (let j in result) {
						result[j] = this.groupBy(result[j], grouping[i]);
					}
				}
				return result;
			},
			...
			getPage1: function () {
				...
				if (Array.isArray(this.rows)) {
					return this.rows.filter((item, i) => 
						i >= this.current.page.size * (this.current.page.number - 1)
						&& i < this.current.page.size * this.current.page.number);
				}
				else {
